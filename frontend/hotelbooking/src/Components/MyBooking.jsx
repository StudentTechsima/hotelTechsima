...
const MyBooking = () => {
  ...
  const navigate = useNavigate();
    ...
    const getAllBooking =async () => {
        try {
            const getRoom = await fetch(`https://hoteltechsima-backend.onrender.com/client/get-my-booking`, {
                method:"GET",
                headers:{
  ...
  const cancelBooking = async (item) => {
    try {
      ...
      if(item.booking_status === 'Canceled'){
        ...
        const res = await fetch(`https://hoteltechsima-backend.onrender.com/client/cancel-booking/${item._id}`, {
          method:"PUT",
          headers:{
    ...
    <table className="booking-table">
      ...
      <tbody>
        {MyBooking.map((item, index) => (
          <tr key={item._id}>
            <td>{index+1}</td>
            <td><img src={`https://hoteltechsima-backend.onrender.com/${item.room_image}`} alt="Room" className="myroom-image" /></td>
