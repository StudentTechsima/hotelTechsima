...
const ViewBookingRoom = () => {
  ...
  const getOrigionalPrice = async () => {
    try {
      const res = await fetch(`https://hoteltechsima-backend.onrender.com/client/origional-room-price/${roomNum ? roomNum : ''}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("usertoken")}`
        }
    ...
    try {
      ...
      if (room.booking_status === 'Canceled') {
        ...
        if (localStorage.getItem('usertoken')) {
          ...
          const res = await fetch(`https://hoteltechsima-backend.onrender.com/client/update-view-room/${id}`, {
            method: "PUT",
            headers: {
    ...
    <>
      <div className="room-details">
        <div className="image-container">
          <img src={`https://hoteltechsima-backend.onrender.com/${room.room_image}`} className="room-image" alt={'room Image'} title={`Room number ${room.room_number}`} />
        </div>
